...
function setupForwarding(api, channel) {
  var mkForwarder = function (namespaceName, method) {
    api[namespaceName][method] = forwardMethod(
      namespaceName,
      method,
      channel,
      function () {
...
function init(options, api) {
  ...
  channel.handler = messages.createMessageHandler(
  ...
  iframe = dom.createFrame(
    options.egnyteDomainURL + "/" + options.forwarderAddress
  );
