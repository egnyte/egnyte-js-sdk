...
function storeFile(
  ...
  stream,
  mimeType /* optional */,
  size /*optional, if we start using multipart*/
) {
  ...
  return promises(true).then(function () {
    ...
    var opts = {
      method: "POST",
      uri:
        requestEngine.getEndpoint() +
        ENDPOINTS.fscontent +
        helpers.encodeURIPath(pathFromRoot),
    };
  ...
  chunker.on("chunkEnd", function (id, next) {
    ...
    if (chunkNumber === 1) {
      if (stream._readableState.pipesCount === 0) {
        ...
        storeFile
          .call(self, pathFromRoot, bufferStream, mimeType, buf.size)
          .then(function (result) {
            defer.resolve(result);
          });
      } else {
        self.internals.startChunkedUpload
          .call(self, pathFromRoot, buf, mimeType)
      ...
      if (stream._readableState.pipesCount === 0) {
        ...
        chunkedUploader.sendChunk(buf, chunkNumber).fail(function (err) {
          //chunk failed. retry?
          defer.reject(err);
        });
...
function getFileStream(pathFromRoot, versionEntryId) {
  ...
  function oneTry() {
    requestEngine
      .retrieveStreamFromRequest(decorate(opts))
