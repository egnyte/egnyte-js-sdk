  ...
  return promises.allSettled(this.chunksPromised).then(function () {
    if (self.num === self.successful) {
      return genericUpload(
        ...
        verify &&
          verify(result.response.headers["x-egnyte-chunk-sha512-checksum"]);
        return {
      ...
      throw new Error(
        "Tried to commit a file with missing chunks (some uploads failed)"
      );
    }
  });
};
...
exports.startChunkedUpload = function (
  pathFromRoot,
  fileOrBlob,
  mimeType,
  verify
