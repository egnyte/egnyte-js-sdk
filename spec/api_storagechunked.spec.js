...
describe("Storage API facade integration", function () {
  ...
  xdescribe("Chunked file upload (SKIPPED: Requires chunks >= 10MB)", function () {
    it("Needs a location to upload to", function (done) {
      eg.API.storage
        .path("/Shared/SDKTests")
        .get()
        .then(function (e) {
      ...
      eg.API.storage
        ...
        .then(function (e) {
          ...
          return eg.API.storage
            .path(testpath)
            .download(null, false /*non binary*/);
        })
        .then(function (xhr) {
          expect(xhr.body).toEqual(
            "[chunk 1 content][chunk 2 content][chunk 3 content][chunk 4 content]"
          );
