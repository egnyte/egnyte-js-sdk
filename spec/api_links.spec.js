...
describe("Link API facade integration", function () {
  ...
  describe("Link methods", function () {
    ...
    it("Needs a file to link to", function (done) {
      // Use /Shared/SDKTests - guaranteed to exist from setup
        ...
        .then(function (e) {
          ...
          if (other.length) {
            eg.API.link
              ...
              .fail(function (e) {
                throw new Error(
                  "Link from the list doesn't seem to exist at all"
                );
              });
          } else {
            done();
    ...
    it("Can destroy a link to file", function (done) {
      eg.API.link
        .removeLink(recentLink.links[0].id)
        .then(function (e) {
