...
describe("API auth", function () {
  ...
  if (
    ...
    describe("Password grant", function () {
      ...
      xit("should be able to log in and use the token (SKIPPED: QA environment API key not configured)", function (done) {
        expect(eg2.API.auth.isAuthorized()).toBe(false);
        eg2.API.auth
          .requestTokenByPassword(APIUsername, APIPassword)
          .then(function () {
