...
describe("Item names / ", function () {
  ...
  var badFiles = ["danglingdot.", "spam.tmp", "Thumbs.db"];
  ...
  describe("Good names:", function () {
    it("BTW. basePath needs to be known at this point", function (done) {
      if (!basePath) {
        // Use /Shared/SDKTests directly - guaranteed to exist from setup
        basePath = "/Shared/SDKTests";
    ...
    goodTestNames.forEach(function (fname) {
      it("BTW. file leftovers from previous tests need to be destroyed", function (done) {
        ...
        eg.API.storage
          ...
          .remove()
          .then(function () {
            done(); // File deleted successfully
          })
          .fail(function (e) {
            // Ignore 404 errors - file doesn't exist, which is fine
            if (e.statusCode === 404) {
              done();
            } else {
