...
describe("Events API facade", function () {
  ...
  describe("Events fetcher", function () {
                ...
                .then(function (resp) {
                  ...
                  if (resp.body && resp.body.events) {
                    for (var i = 0; i < resp.body.events.length; i++) {
                      expect(resp.body.events[i].data.target_path).not.toEqual(
                        filePath
                      );
                    }
                  }
    ...
    xit("Should not get app's own note events (SKIPPED: Timeout in QA environment)", function (done) {
      var filePath = testpath + "/candy.txt";
      var noteId;
    ...
    if (typeof OtherUsername !== "undefined") {
      xit("Should not get user's own events (SKIPPED: Test times out in QA environment)", function (done) {
        ...
        filteredEventsSource
          ...
          .then(function (startEventId) {
            return egnyteDelay(eg, null, 1000)
              ...
              .then(function (e) {
                return egnyteDelay(eg, e, 1000).then(function () {
                  return eg.API.storage
                    .impersonate({
                      username: OtherUsername,
                    })
                    .path(otherFilePath)
                    .storeFile(getFileContent("sour as ...much as possible"));
                });
              })
              .then(function (e) {
