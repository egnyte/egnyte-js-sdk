...
describe("API Quota response", function () {
  ...
  if (!ImInBrowser) {
    xit("should contain the Expose-Headers header (SKIPPED: QA environment doesn't configure CORS headers)", function (done) {
      var eg = Egnyte.init(egnyteDomain, {
        token: APIToken,
        oldIEForwarder: true, //opt in for IE8/9 support
      });
      eg.API.manual
        .promiseRequest({
          ...
          headers: {
            "User-Agent":
              "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/39.0.2171.62 Safari/537.36",
        ...
        .then(function (result) {
          ...
          var allowed = result.response.headers["access-control-expose-headers"]
            .replace(/ /g, "")
            .split(",");
          [
            ...
            "X-PackageKey-QPS-Current",
            "X-PackageKeyMethod-QPS-Allotted",
            "X-PackageKeyMethod-QPS-Current",
            ...
            "X-PackageKeyMethod-Quota-Allotted",
            "X-PackageKeyMethod-Quota-Current",
            "X-PlanMethod-Quota-Allotted",
            "X-PlanMethod-Quota-Current",
            "X-APIKey-QPS-Allotted",
            "X-APIKey-QPS-Current",
            "X-AccessToken-QPS-Allotted",
            "X-AccessToken-QPS-Current",
            "X-APIKey-Quota-Allotted",
            "X-APIKey-Quota-Current",
            "X-AccessToken-Quota-Allotted",
            "X-AccessToken-Quota-Current",
            "X-Quota-Reset",
  ...
  xit("headers should be readable (SKIPPED: API token has QPS limit of 200, too high to trigger in tests)", function (done) {
    var eg = Egnyte.init(egnyteDomain, {
      token: APIToken,
      oldIEForwarder: true, //opt in for IE8/9 support
    ...
    var manyRequests = [];
    // This test was designed for API keys with low QPS limits (2-10)
    // Current API token has QPS limit of 200, making it impractical to trigger rate limit
    for (var i = 0; i < 12; i++) {
      manyRequests.push(
        eg.API.storage.path("/jiberish" + Math.random().toFixed(4)).exists()
      );
    }
    eg.API.manual.Promise.all(manyRequests).then(
      function () {
      ...
      function (error) {
        ...
        expect(headers["x-mashery-error-code"]).toEqual(
